...
<body>
    ...
    <section>
        ...
        <p><em>If you have any suggestions for how we can do better at this,  do let us know!</em></p>
    </section>

    <h2>Payment Options</h2>
    <section>
        <p>
            We take <strong>cash,</strong> <strong>credit</strong> (Apple Pay accepted), and <strong>Cornell Card.</strong> No BRBs or MealChoice–sorry! We wish we could. We do offer <strong>discounts</strong> to grad students and faculty, and to anyone who brings their own mug or bowl–so BYOB!
        </p>

        <p>If you would like to start an <strong>account</strong> for large orders of coffee and/or pastries to be placed ahead of time, email us at <strong>templeofzeus@cornell.edu</strong> and we'll help you set one up, or place an order below and select "Create New Account."" Those eligible to create an account are <em>academic departments</em> and <em>recognized student organizations.</em> Scroll down to find the <strong>order form for your convenience!</strong></p>
    </section>
    ...
    <h2>Order Ahead</h2>
        <?php if ($show_form) { ?>
            <h3>Place an account order here:</h3>
                <p><em>For use by academic departments and student organizations.</em></p>
                <p>Want to start an account to order coffee or pastries from Zeus for your meetings or events?</p>
                <p>Email us at <strong>templeofzeus@cornell.edu</strong> or fill out the order form below and we'll help you set up an account to do just that!</p>
                <form id="OrderForm" method="post" novalidate>

                <div>
                    <label for="account">Account: </label>
                        <select name="account" id="account" required>
                            <option value="default"><?php echo htmlspecialchars($account); ?></option>
                    ...
                    <p class="errorContainer <?php echo ($show_account_feedback) ? '' : 'hidden'; ?>">Please select an account to be charged, or select "Create New Account."</p>
                </div>
                ...
                <div>
                    ...
                    <p class="errorContainer <?php echo ($show_email_feedback) ? '' : 'hidden'; ?>">Please enter a valid email address.</p>
                </div>

                <div>
                    <label for="contactPhone">Phone Number: </label>
                    <input id="contactPhone" name="contactPhone" placeholder="XXXXXXXXXX" value="<?php echo htmlspecialchars($phone); ?>" required/>
                    <p class="errorContainer <?php echo ($show_phone_feedback) ? '' : 'hidden'; ?>">Please enter a phone number in case we need to contact you (no dashes please).</p>
                </div>

                <div>
                    <label for="date">Date: </label>
                    <input type="date" id="date" name="date" value="<?php echo htmlspecialchars($date); ?>" required>
                    <p class="errorContainer <?php echo ($show_date_feedback) ? '' : 'hidden'; ?>">Please select the date for when the order should be filled.</p>
                </div>
                ...
                <div>
                    ...
                    <p class="errorContainer <?php echo ($show_pickup_feedback) ? '' : 'hidden'; ?>">Please select a time during open hours (7:30 AM-6:00 PM).</p>
                </div>
                ...
                <div>
                    ...
                    <p class="errorContainer <?php echo ($show_request_feedback) ? '' : 'hidden'; ?>">Please let us know what you'd like to order.</p>
                </div>
            ...
            <?php } else { ?>
                <h2>Order placed for <?php echo $account ?> </h2>
                <ul>
                    <li>Email: <?php echo $email?></li>
                    <li>Phone: <?php echo $phone?></li>
                    <li>Date: <?php echo $date?></li>
                    <li>Pickup Time: <?php echo $pickup?></li>
                    <li>Order Details: <?php echo $request?></li>
                </ul>
            <?php } ?>
        </section>
